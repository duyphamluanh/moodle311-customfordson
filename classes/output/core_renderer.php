...
' . $jitsibuttontext . ' </span></a>';
    ...
    */
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        // Elo: show/hide guidelines
        $context->displayguidelines = $PAGE->theme->settings->guidelines == 1;
        // Elo: extra buttons
        if(!empty($PAGE->theme->settings->contentbuttonextraoneurl)){
            $context->namebuttonone = $PAGE->theme->settings->contentbuttonextraone;
            $context->urlbuttonone = $PAGE->theme->settings->contentbuttonextraoneurl;
            if(empty($context->namebuttonone)){
                $context->namebuttonone = $this->web_regular_expressions_get_domains($context->urlbuttonone)[1];
            }
        }
        if(!empty($PAGE->theme->settings->contentbuttonextratwourl)){
            $context->namebuttontwo = $PAGE->theme->settings->contentbuttonextratwo;
            $context->urlbuttontwo = $PAGE->theme->settings->contentbuttonextratwourl;
            if(empty($context->namebuttontwo)){
                $context->namebuttontwo = $this->web_regular_expressions_get_domains($context->urlbuttontwo)[1];
            }
        }
        if(!empty($PAGE->theme->settings->contentbuttonextrathreeurl)){
            $context->namebuttonthree = $PAGE->theme->settings->contentbuttonextrathree;
            $context->urlbuttonthree = $PAGE->theme->settings->contentbuttonextrathreeurl;
            if(empty($context->namebuttonthree)){
                $context->namebuttonthree = $this->web_regular_expressions_get_domains($context->urlbuttonthree)[1];
            }
        }
        // Elo: Technical support contacts
        if(!empty($PAGE->theme->settings->technicalsupportmobile) || !empty($PAGE->theme->settings->technicalsupportemail)){
            $context->technicalsupportcontact = true;
        }
        if(!empty($PAGE->theme->settings->technicalsupportmobile)){
            $context->technicalsupportmobile = $PAGE->theme->settings->technicalsupportmobile;
        }
        if(!empty($PAGE->theme->settings->technicalsupportemail)){
            $context->technicalsupportemail = $PAGE->theme->settings->technicalsupportemail;
        }
    ...
    public function web_regular_expressions_get_domains($context){
        $matches = [];
        $pattern = '/^(?:https?:\/\/)?(?:[^@\/\n]+@)?(?:www\.)?([^:\/?\n]+)/imu';
        preg_match_all ( $pattern , $context, $matches, PREG_SET_ORDER, 0);
        return $matches[0];
    }
    ...
    public function show_student_navbarcolor() {
     ...
     /**
     * Returns a search box. Elo search
     *
    ...
    public function search_box_custom($id = false) {
        ...
        $uniqid = ($id == false) ? uniqid() : clean_param($id, PARAM_ALPHANUMEXT);
            
        $data = [
            'action' => new moodle_url($CFG->wwwroot . '/course/search.php'),
            'unique' => $uniqid,
        ];
        return $this->render_from_template('theme_fordson/search-box-custom', $data);
    }
}